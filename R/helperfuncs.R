...
# ---------------------------------------------------------------------------- #
# Canonical regime labeling.
#
# Estimation is invariant to permuting regime labels, so a fitted model's
# labeling is arbitrary. This orders state 1 as the least persistent regime and
# the last state as the most persistent, needed wherever a fixed labeling
# convention matters (e.g. burnin = 0 simulation, which starts every draw in
# state 1). Never called automatically.
# ---------------------------------------------------------------------------- #
...
# Scalar-per-regime sigma proxy for the ordering tie-break: the (1,1) entry of
# each regime's covariance, valid whether sigma is a list of q x q matrices
# (multivariate) or a k x 1 matrix of variances (univariate).
.canon_sigma_key <- function(sigma, k){
  if (is.list(sigma)) vapply(sigma, function(s) s[1, 1], numeric(1)) else as.numeric(sigma)[seq_len(k)]
}

# Permute a regime-indexed field by 'perm', dispatching on its shape: sigma/
# stdev/beta are a list of k matrices for multivariate classes but a plain
# matrix for univariate ones.
.canon_permute_field <- function(x, perm, along){
  if (is.null(x)) return(x)
  if (is.list(x) && !is.data.frame(x)) return(x[perm])
  if (along == "rows") return(if (is.matrix(x)) x[perm, , drop = FALSE] else x[perm])
  if (along == "cols") return(if (is.matrix(x)) x[, perm, drop = FALSE] else x[perm])
...
#'   \code{resid} and \code{fitted} are unchanged (a relabeling does not change
#'   the likelihood). Fields with no regime axis (\code{phi}, \code{betaZ},
#'   \code{y}, \code{Z}, \code{n}, \code{p}, \code{q}, \code{k}, \code{control},
#'   the \code{theta_*_ind} position maps, \code{theta_0}, \code{init_used},
#'   \code{trace}) are unchanged.
#'
...
canonicalize_regimes <- function(mdl, by = "persistence"){
  ...
  # Read P from theta, not mdl$P: mdledit() products can carry a stale mdl$P.
  ...
  # Order by persistence, then each series' mean, then sigma, then the original
  ...
  names(theta_new) <- names(mdl$theta)
  ...
  out$pinf <- .canon_permute_field(mdl$pinf, perm, "rows")   # (k x 1) matrix on every real fit -- kept that way, not flattened
  ...
  if (!is.null(mdl$Hess)){
    # mdl$Hess is already in .thetaSE_reduced()'s free-coordinate space (P's
    # columns sum to 1, so the full Hessian is singular and gets projected down
    # before storage), not the full theta space theta_idx indexes. Re-express the
    # permutation in that reduced space: Pi J = J A for A = solve(t(J)J) t(J) Pi J,
    # exact because permuting regimes maps the constraint tangent space span(J) to
    # itself. The Hessian then transforms by the congruence A' Hess A.
    setup <- .reduced_P_setup(k, npar)
    J <- setup$J
