...
MCLRTest <- function(Y, ar, k0, k1, msmu = TRUE, msvar = TRUE, control = list()){
  ...
  if(con[["workers"]]>0){
    LRN <- LR_samp_dist_par(mdl_h0, k1, msmu, msvar, con[["N"]], con[["maxit"]], con[["thtol"]], con[["burnin"]], con[["finite_max_init"]], con[["dist_converge_iter"]], con[["init_val_try_dist"]], con[["workers"]])
  }else{
...
BootLRTest <- function(Y, ar, k0, k1, msmu = TRUE, msvar = TRUE, control = list()){
  ...
  # ----- Simulate sample null distribution
  if(con[["workers"]]>0){
    LRN <- LR_samp_dist_par(mdl_h0, k1, msmu, msvar, con[["B"]], con[["maxit"]], con[["thtol"]], con[["burnin"]], con[["finite_max_init"]], con[["dist_converge_iter"]], con[["init_val_try_dist"]], con[["workers"]])
  }else{
    LRN <- LR_samp_dist(mdl_h0, k1, msmu, msvar, con[["B"]], con[["maxit"]], con[["thtol"]], con[["burnin"]], con[["finite_max_init"]], con[["dist_converge_iter"]], con[["init_val_try_dist"]]) 
  }
  # ----- Compute p-value
